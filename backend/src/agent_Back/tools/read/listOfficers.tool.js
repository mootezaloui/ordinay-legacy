...
async function handler({ query = null, status = null, limit = 50 } = {}) {
  let officers = officersService.list();

  if (status) {
    const normalized = String(status).toLowerCase();
    officers = officers.filter(
      (officer) => String(officer.status || '').toLowerCase() === normalized,
    );
  }
  ...
  const limited = officers.slice(0, limit);
...
module.exports = {
  ...
  description: 'List officers (bailiffs/huissiers) with optional filters',
  inputSchema,
  outputSchema,
  reversibility: true,
  sideEffects: false,
