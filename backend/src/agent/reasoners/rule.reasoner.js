...
class RuleReasoner extends BaseReasoner {
  ...
  async chat({ message }) {
    ...
    return {
      type: 'chat',
      message: llmResponse || 'I apologize, but I am unable to respond right now. Please try again.',
      timestamp,
  ...
  _buildEnrichedExplanation(message, context, timestamp) {
    ...
    if (context.overdueTasks && context.overdueTasks.length > 0) {
      ...
      context.overdueTasks.slice(0, 5).forEach(task => {
        details.push(`  - ${task.title} (${task.priority}, ${Math.round(task.days_overdue)} days overdue)`);
      });
      ...
      if (context.overdueAnalysis) {
        ...
        if (analysis.averageDaysOverdue) {
          details.push(`  Average days overdue: ${analysis.averageDaysOverdue}`);
        }
      }
    ...
    if (details.length === 0) {
      ...
      details.push('The requested information could not be located in the system.');
