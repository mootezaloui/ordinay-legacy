...
async function* streamChatResponse(message, signal) {
  console.log('[LLM STREAM] Starting stream request to:', `${LLM_BASE_URL}/api/generate`);

  // Create a combined abort controller for timeout and caller cancellation
  const abortController = new AbortController();
  const timeoutId = setTimeout(() => {
    console.log('[LLM STREAM] Request timeout after', LLM_TIMEOUT, 'ms, aborting...');
  ...
  try {
    ...
    console.log('[LLM STREAM] Response status:', response.status, response.ok ? 'OK' : 'FAILED');
    if (!response.ok) {
...
async function streamChatWithCallbacks(message, callbacks, signal) {
  console.log('[LLM STREAM CB] Starting stream request to:', `${LLM_BASE_URL}/api/generate`);
  ...
  try {
    ...
    while (true) {
      ...
      for (const line of lines) {
        try {
          const data = JSON.parse(line);
          console.log('[LLM STREAM CB] Parsed data, response:', data.response ? `"${data.response}"` : '(empty)', 'done:', data.done);
          if (data.response) {
          ...
          console.log('[LLM STREAM CB] JSON parse error:', parseErr.message, 'line:', line.slice(0, 50));
        }
      }
    }
