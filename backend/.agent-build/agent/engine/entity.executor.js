...
const DOCUMENT_PARENT_FIELD_ALIASES = [
    ["client_id", "clientId"],
...
class EntityExecutor {
    ...
    rules = new domain_1.DomainRuleProfile();
    storageModule;
    rendererModule;
    dbModule;
    ...
    async executeWorkflow(rootOperation, steps, context) {
        const stepResults = [];
        ...
        for (const step of steps) {
            ...
            if (!validation.allowed) {
                const errorMessage = validation.notes[0] ||
        ...
        const lastSuccessful = stepResults.length > 0
    ...
    async executeSingleOperationWithValidation(operation, context) {
        const validation = await this.rules.validateOperation(operation);
        if (!validation.allowed) {
            return {
                ok: false,
                errorCode: "DOMAIN_RULE_BLOCKED",
                errorMessage: validation.notes[0] ||
    ...
    async executeSingleOperation(operation, context) {
        ...
        try {
            if (operation.operation === "create") {
                ...
                const preparedDocument = entityType === "document"
                ...
                const createPayload = entityType === "document" && preparedDocument?.ok === true
    ...
    async preflightCreateOperation(entityType, payload) {
        ...
        if (entityType === "task" || entityType === "session" || entityType === "mission") {
            const caseScopeLinks = parentLinks.filter((row) => CASE_PARENT_ENTITY_TYPES.has(row.entityType));
            if (caseScopeLinks.length === 0) {
                return this.createPreflightFailure("EXEC_PRECONDITION_LINK_MISSING", `Cannot create ${entityType} without a linked dossier or lawsuit.`, {
                    category: "link",
            ...
            if (caseScopeLinks.length > 1) {
                return this.createPreflightFailure("EXEC_PRECONDITION_LINK_CONFLICT", `Cannot create ${entityType} with both dossier and lawsuit links at the same time.`, {
                    category: "link",
        ...
        if (entityType === "document") {
            if (parentLinks.length !== 1) {
                return this.createPreflightFailure("EXEC_PRECONDITION_LINK_CONFLICT", "Cannot create document without exactly one linked parent entity.", {
                    category: "link",
                    reason: "document_parent_invalid",
                    entityType,
                    hint: "Link this document to exactly one parent (client, dossier, lawsuit, mission, task, session, personal task, financial entry, or officer).",
                });
            }
            const filePath = asNonEmptyString(normalizedPayload.file_path);
            if (!filePath) {
                return this.createPreflightFailure("EXEC_PRECONDITION_STORAGE_SOURCE_MISSING", "Cannot create document without a resolved file path.", {
                    category: "storage",
                    reason: "file_path_missing",
                    entityType,
                    hint: "Regenerate or reattach the draft so the system can persist the document file before creation.",
                });
            }
            if (!_fs.existsSync(filePath)) {
                return this.createPreflightFailure("EXEC_PRECONDITION_STORAGE_PATH_MISSING", "Cannot create document because the prepared file path does not exist.", {
                    category: "storage",
                    reason: "file_path_not_found",
                    entityType,
                    filePath,
                    hint: "Regenerate the document content and try again so a valid file is created.",
                });
            }
        }
        for (const link of parentLinks) {
            ...
            if (!exists) {
                return this.createPreflightFailure("EXEC_PRECONDITION_LINK_NOT_FOUND", `Cannot create ${entityType} because linked ${link.entityType} ${String(link.entityId)} no longer exists.`, {
                    category: "link",
                    ...
                    hint: `Choose an existing ${link.entityType} and confirm again.`,
                });
            }
        }
        return null;
    }
    normalizeParentAliasFields(payload) {
    ...
    async parentEntityExists(entityType, entityId) {
        const service = this.getService(entityType);
    ...
    createPreflightFailure(errorCode, errorMessage, errorDetails) {
        return {
            ok: false,
            errorCode,
            errorMessage,
    ...
    async prepareDocumentCreatePayload(rawPayload, context) {
        ...
        if (!filePath) {
            if (!generationToken) {
                return {
                    ok: false,
                    failure: this.createPreflightFailure("EXEC_PRECONDITION_STORAGE_SOURCE_MISSING", "Cannot create document without a storage source (file path or supported generation token).", {
                        category: "storage",
                        reason: "storage_source_missing",
                        entityType: "document",
                        hint: "Regenerate the draft or attach a file so the document can be persisted before confirmation.",
                    }),
                };
            }
                ...
                return {
                    ok: false,
                    failure: this.createPreflightFailure("EXEC_PRECONDITION_STORAGE_TOKEN_UNSUPPORTED", `Cannot create document from unsupported generation token "${generationToken}".`, {
                        category: "storage",
                        reason: "unsupported_generation_token",
                        entityType: "document",
                        token: generationToken,
                        hint: "Use the current draft save flow or provide a concrete file path before confirming.",
                    }),
                };
            }
            else {
                if (!draftSnapshot) {
                    return {
                        ok: false,
                        failure: this.createPreflightFailure("EXEC_PRECONDITION_DRAFT_SNAPSHOT_MISSING", "Cannot create document because the draft snapshot is missing.", {
                            category: "storage",
                            reason: "draft_snapshot_missing",
                            entityType: "document",
                            hint: "Regenerate or reopen the draft in this conversation, then confirm again.",
                        }),
                    };
                }
                let rendered = null;
                try {
                    ...
                    return {
                        ok: false,
                        failure: this.createPreflightFailure("EXEC_PRECONDITION_STORAGE_RENDER_FAILED", "Cannot create document because draft rendering failed.", {
                            category: "storage",
                            reason: "draft_render_failed",
                            entityType: "document",
                            detail,
                            hint: "Try regenerating the draft and confirm again. If this persists, check rendering service availability.",
                        }),
                    };
                }
                if (!rendered) {
                    return {
                        ok: false,
                        failure: this.createPreflightFailure("EXEC_PRECONDITION_STORAGE_RENDER_FAILED", "Cannot create document because draft rendering returned no output.", {
                            category: "storage",
                            reason: "draft_render_empty",
                            entityType: "document",
                            hint: "Regenerate the draft and retry confirmation.",
                        }),
                    };
                }
                ...
                if ((typeof payload.size_bytes !== "number" || !Number.isFinite(payload.size_bytes)) &&
        ...
        if (parentCount !== 1) {
            return {
                ok: false,
                failure: this.createPreflightFailure("EXEC_PRECONDITION_LINK_CONFLICT", "Cannot create document without exactly one linked parent entity.", {
                    category: "link",
                    reason: "document_parent_invalid",
                    entityType: "document",
                    hint: "Keep exactly one parent link (client, dossier, lawsuit, mission, task, session, personal task, financial entry, or officer).",
                }),
            };
        }
        if (!asNonEmptyString(payload.file_path)) {
            return {
                ok: false,
                failure: this.createPreflightFailure("EXEC_PRECONDITION_STORAGE_SOURCE_MISSING", "Cannot create document without a resolved file path.", {
                    category: "storage",
                    reason: "file_path_missing",
                    entityType: "document",
                    hint: "Regenerate or attach the document content so a file path is available.",
                }),
            };
        }
        ...
        if (!asNonEmptyString(payload.title)) {
            return {
                ok: false,
                failure: this.createPreflightFailure("EXEC_PRECONDITION_PAYLOAD_INVALID", "Cannot create document without a title.", {
                    category: "validation",
                    reason: "title_missing",
                    entityType: "document",
                    hint: "Provide a document title in the request and confirm again.",
                }),
            };
        }
        return {
    ...
    normalizeDocumentAliasFields(payload) {
        if (!isRecord(payload)) {
            return;
        }
    ...
    stripDocumentBridgeFields(payload) {
    ...
    resolveDocumentStorageToken(payload) {
        for (const key of DOCUMENT_STORAGE_SOURCE_KEYS) {
    ...
    readDraftSnapshot(payload) {
        ...
        const formality = formalityRaw === "formal" || formalityRaw === "standard" || formalityRaw === "casual"
            ? formalityRaw
            : "formal";
        ...
        const versionValue = typeof raw.version === "number" && Number.isFinite(raw.version)
    ...
    readDraftProvenance(payload) {
        ...
        const versionRaw = typeof row.draftVersion === "number" && Number.isFinite(row.draftVersion)
    ...
    async renderDraftSnapshot(draft) {
        ...
        const fileHash = _crypto
            ...
            .digest("hex")
            .slice(0, 12);
        const outputPath = _path.join(documentsRoot, "generated", daySegment, `agent_draft_${Date.now()}_${fileHash}.pdf`);
    ...
    renderDraftSnapshotText(sections) {
        return sections
            .map((section) => {
            const label = asNonEmptyString(section.label);
            const text = asNonEmptyString(section.text);
    ...
    persistDocumentDraftProvenance(documentId, provenance) {
        ...
        try {
            ...
            const nextArtifact = {
                ...
                draft_provenance: {
                    ...
                    draft_version: typeof provenance.draftVersion === "number" && Number.isFinite(provenance.draftVersion)
                        ? provenance.draftVersion
                        : null,
                    persisted_at: new Date().toISOString(),
                },
            };
            db.prepare("UPDATE documents SET artifact_json = @artifact_json, updated_at = CURRENT_TIMESTAMP WHERE id = @id").run?.({
                id: entityId,
    ...
    getDocumentStorageModule() {
        if (this.storageModule !== undefined) {
            return this.storageModule;
        }
        try {
            const resolved = _path.resolve(__dirname, "..", "..", "..", "src", "services", "documentStorage");
    ...
    getDocumentRendererModule() {
        if (this.rendererModule !== undefined) {
            return this.rendererModule;
        }
        try {
            const resolved = _path.resolve(__dirname, "..", "..", "..", "src", "services", "documentGeneration", "renderer.service");
            const loaded = require(resolved);
    ...
    getDbModule() {
        ...
        try {
            const resolved = _path.resolve(__dirname, "..", "..", "..", "src", "db", "connection");
            const loaded = require(resolved);
...
exports.EntityExecutor = EntityExecutor;
...
function isPlanOperationType(value) {
...
function toOperationFromStep(step) {
    return {
...
function unsupportedOperation(entityType, operation) {
    return {
        ok: false,
        errorCode: "UNSUPPORTED_OPERATION",
...
function normalizeExecutionError(error) {
    ...
    const code = lowered.includes("required") ||
...
function parentEntityTypeFromField(field) {
    const normalized = String(field || "").trim().toLowerCase();
    if (normalized === "client_id")
        return "client";
    if (normalized === "dossier_id")
        return "dossier";
    if (normalized === "lawsuit_id")
        return "lawsuit";
    ...
    if (normalized === "session_id")
        return "session";
    if (normalized === "personal_task_id")
        return "personal_task";
    if (normalized === "financial_entry_id")
        return "financial_entry";
    if (normalized === "officer_id")
        return "officer";
    return null;
}
function countDocumentParentLinks(payload) {
    let count = 0;
