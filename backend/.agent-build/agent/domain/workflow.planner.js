...
exports.DomainWorkflowPlanner = void 0;
...
class DomainWorkflowPlanner {
    rules;
    graphAnalyzer;
    serviceCache = new Map();
    constructor(rules = new rule_profile_1.DomainRuleProfile(), graphAnalyzer = new graph_analyzer_1.DomainGraphAnalyzer(rules)) {
        this.rules = rules;
    ...
    async expand(input) {
        ...
        if (validation.allowed) {
            const preview = buildSingleEntityPreview(rootOperation, basePreview, rootLabel, rootCurrentValues);
        ...
        if (rootOperation.operation === "update" &&
        ...
        else if (rootOperation.operation === "update" &&
            ...
            steps.push(buildUpdateOrDeleteStep({
                id: "step_root",
            ...
            const reopenFlowSteps = buildAncestorReopenWorkflowSteps(rootOperation, blockerCounts, rootSnapshot);
    ...
    async readEntitySnapshot(entityType, entityId) {
        if (entityId == null)
            return null;
...
exports.DomainWorkflowPlanner = DomainWorkflowPlanner;
function normalizeRootOperation(operation) {
    ...
    if (operation.entityId != null)
        normalized.entityId = operation.entityId;
    ...
    if (isRecord(operation.changes)) {
        ...
        if (Object.prototype.hasOwnProperty.call(nextChanges, "status")) {
            const canonical = (0, status_normalizer_1.canonicalizeTargetStatus)(entityType, nextChanges.status);
            if (canonical) {
...
function buildClientInactiveWorkflowSteps(blockers, rootOperation, userMessage) {
    const steps = [];
    ...
    const shouldSettleReceivables = blockers.unpaidReceivables.length > 0 && looksLikeSettleReceivables(userMessage);
...
function buildDossierCloseWorkflowSteps(blockers, rootOperation, userMessage) {
    const steps = [];
    for (const row of blockers.tasks) {
        const id = asEntityId(row.id);
        if (id == null)
...
function buildLawsuitCloseWorkflowSteps(blockers, rootOperation) {
    const steps = [];
    for (const row of blockers.tasks) {
        const id = asEntityId(row.id);
        if (id == null)
...
function buildAncestorReopenWorkflowSteps(rootOperation, blockerCounts, rootSnapshot) {
    ...
    const linkedEntityType = (0, status_normalizer_1.normalizeStatus)(source.entityType ?? source.entity_type).replace(/_/g, "");
...
function resolveOperationSource(operation, rootSnapshot) {
    return {
...
function buildSingleEntityPreview(operation, basePreview, rootLabel, rootCurrentValues) {
    return {
        ...(basePreview || {}),
        scope: "single_entity",
        root: {
...
function buildWorkflowPreview(params) {
    const { rootOperation, steps, basePreview, notes, requiresUserDecision, decisionOptions, rootLabel, rootCurrentValues, } = params;
...
function buildPrimaryChanges(operation, defaultEntityType, rootCurrentValues) {
    if (!isRecord(operation.changes)) {
        return [];
    }
    const changes = [];
...
function summarizeCascadeSteps(steps) {
        ...
        for (const step of groupSteps.slice(0, 3)) {
            if (isRecord(step.changes)) {
                for (const [field, value] of Object.entries(step.changes)) {
                    ...
                    const diff = isRecord(value) ? value : null;
                    examples.push({
                        entityType,
...
function buildUpdateOrDeleteStep(params) {
    return {
        id: params.id,
        actionType: params.operation === "create"
            ? "CREATE_ENTITY"
...
function sequenceWithDependencies(steps) {
    const normalized = [];
    let previousId;
    let counter = 1;
    for (const raw of steps) {
        ...
        normalized.push({
            ...raw,
            id: nextId,
            dependsOn: previousId && (!Array.isArray(raw.dependsOn) || raw.dependsOn.length === 0)
                ? [previousId]
                : raw.dependsOn,
        });
        previousId = nextId;
...
function statusRequested(operation, expected) {
    if (!isRecord(operation.changes))
        return false;
    const requestedStatus = (0, status_normalizer_1.canonicalizeTargetStatus)(operation.entityType, operation.changes.status);
    return requestedStatus === expected;
}
function assertEntityId(operation) {
    if (operation.entityId == null) {
...
function looksLikeSettleReceivables(userMessage) {
    const normalized = (0, status_normalizer_1.normalizeStatus)(userMessage).replace(/_/g, " ");
    return (/mark .*invoice.*paid/.test(normalized) ||
...
function looksLikeForceDelete(userMessage) {
    const normalized = (0, status_normalizer_1.normalizeStatus)(userMessage).replace(/_/g, " ");
    return (/force delete/.test(normalized) ||
...
function resolveCurrentValuesFromSnapshot(changes, snapshot) {
    if (!snapshot || !isRecord(changes))
        return undefined;
    const current = {};
...
function resolveEntityDisplayLabel(entityType, entityId, snapshot) {
    if (snapshot) {
        const preferred = [
            snapshot.name,
            snapshot.title,
...
function applyLinkResolutionPreview(preview, diagnostic) {
...
function toLinkResolutionPreview(diagnostic) {
    ...
    const source = diagnostic.source && typeof diagnostic.source === "string" && diagnostic.source.trim().length > 0
...
function resolveLinkResolutionTarget(diagnostic) {
    const directType = typeof diagnostic.entityType === "string" && diagnostic.entityType.trim().length > 0
    ...
    const findCandidateLabel = (type, id) => {
        const match = candidates.find((row) => row &&
    ...
    if (directType && hasDirectId) {
        const directId = diagnostic.entityId;
    ...
    const firstCandidate = candidates.find((row) => row &&
        typeof row.entityType === "string" &&
        row.entityType.trim().length > 0 &&
        (typeof row.entityId === "number" || typeof row.entityId === "string"));
    if (!firstCandidate) {
...
function describeLinkResolutionSource(status, source, userSpecified) {
    if (userSpecified) {
...
function formatLinkingTargetLabel(target) {
    if (!target)
        return null;
    ...
    const reference = typeof target.label === "string" && target.label.trim().length > 0
        ? target.label.trim()
        : `${entityLabel} #${String(target.entityId)}`;
    return `${entityLabel}: ${reference}`;
}
function toLinkResolutionPreviewMessage(diagnostic, linking) {
