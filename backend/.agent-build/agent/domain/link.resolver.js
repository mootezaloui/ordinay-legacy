...
const LINK_FIELD_BY_ENTITY_TYPE = {
    client: "client_id",
    dossier: "dossier_id",
    lawsuit: "lawsuit_id",
    mission: "mission_id",
...
class LinkResolver {
    resolve(operation, context) {
        ...
        if (explicitLinks.length === 1) {
            ...
            const explicitLabel = resolveContextCandidateLabel(explicit.entityType, explicit.entityId, context);
            ...
            return {
                ...
                diagnostic: {
                    ...
                    message: `Parent link preserved from payload: ${explicit.entityType} ${String(explicit.entityId)}.`,
                },
            };
        }
        if (explicitLinks.length > 1) {
            ...
            return {
                status: "ambiguous",
                operation: { ...normalized, payload },
                message: `Create ${normalized.entityType} includes multiple parent links in payload. ` +
                    "Keep exactly one parent reference.",
                diagnostic: {
                    status: "ambiguous",
                    reason: "multiple_payload_links",
                    candidates,
                    message: `Multiple payload parent links detected for create ${normalized.entityType}.`,
                },
            };
        }
        ...
        if (contextCandidates.length > 1) {
            ...
            return {
                status: "ambiguous",
                operation: { ...normalized, payload },
                message: `Could not determine a single parent for create ${normalized.entityType}. ` +
                    `Candidates: ${labelParts.join(", ")}.`,
                diagnostic: {
                    status: "ambiguous",
                    reason: "multiple_context_candidates",
                    candidates: contextCandidates,
                    message: `Multiple parent candidates detected for create ${normalized.entityType}.`,
                },
            };
        }
        ...
        return {
            ...
            diagnostic: {
                ...
                candidates: [winner],
                message: `Resolved parent link for create ${normalized.entityType}: ` +
...
function collectExplicitLinks(payload, allowedLinkTypes) {
    const links = [];
...
function collectContextCandidates(allowedLinkTypes, context) {
    const candidates = [];
    ...
    for (const row of sortActiveEntities(activeRows)) {
        ...
        if (!entityType || entityId == null)
            continue;
        if (!allowedLinkTypes.includes(entityType))
            continue;
...
function resolveContextCandidateLabel(entityType, entityId, context) {
    const activeRows = Array.isArray(context.activeEntities) ? context.activeEntities : [];
...
function sortActiveEntities(rows) {
    const copy = [...rows];
    copy.sort((a, b) => {
